...
export class CorrecaoLivroUseCase {

    constructor(){}

    async handle(seloXml,livroAtt,caminhoDoArquivoSelo): Promise<string> {
              ...
              if (path.extname(caminhoDoArquivoSelo) === '.xml') {
                  xml2js.parseString(seloXml, async (err, result) => {
                      if (err) {
                          ...
                          const livroSemEspaco = livroAtt.trim().split('.').join('');

  
                          if (livroSemEspaco) {
                            
                            result.PrestacaoContas.AtoPraticado[0].Livro[0] = `${livroSemEspaco}`;
