...
export class CorrecaoFrjUseCase {

    constructor(){}

    async handle(seloXml,frj,caminhoDoArquivoSelo): Promise<string> {
      return new Promise<string>((resolve, reject) => {
        ...
        try {
          if (path.extname(caminhoDoArquivoSelo) === '.xml') {
            ...
            const frjComPonto = frj.replace(',','.')

            if(frj){
              
              result.PrestacaoContas.AtoPraticado[0].ValorFRJ= `${frjComPonto}`;
