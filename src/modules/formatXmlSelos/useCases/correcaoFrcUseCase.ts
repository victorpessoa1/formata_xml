...
export class CorrecaoFrcUseCase {

    constructor(){}

    async handle(seloXml,frc,caminhoDoArquivoSelo): Promise<string> {
      return new Promise<string>((resolve, reject) => {
        ...
        try {
          if (path.extname(caminhoDoArquivoSelo) === '.xml') {
            ...
            const frcComPonto = frc.replace(',','.');
            
            if(frc){
              
              result.PrestacaoContas.AtoPraticado[0].ValorFRC[0] = `${frcComPonto}`;
